...
s.prototype.parse = function(t) {
    ...
    i.id && !this.xml && (this.options.useAnchor ? this.expose() : "img" !== t.name && "a" !== t.name && "video" !== t.name && "audio" !== t.name && (i.id = void 0)), 
    ...
    "table" !== t.name && parseInt(c.width) > h && (c["max-width"] = "100%", c["box-sizing"] = "border-box"), 
    a.blockTags[t.name]) t.name = "div"; else if (a.trustTags[t.name] || this.xml) if ("a" === t.name || "ad" === t.name) this.expose(); else if ("video" === t.name || "audio" === t.name) t.children = void 0; else if ("ul" !== t.name && "ol" !== t.name || !t.c) {
        if ("table" === t.name) {
                    ...
                    for (var b = 1, w = 0; w < m[x - 1].children.length; w++, b++) {
                        ...
                        if ("td" === k.name || "th" === k.name) {
                            ...
                            b += parseInt(k.attrs.colspan) - 1), k.attrs.rowspan) {
                                ...
                                for (var C = 1; C < k.attrs.rowspan; C++) for (var S = 0; S < (k.attrs.colspan || 1); S++) y[x + C + "." + (b - S)] = 1;
                            }
            ...
            if (this.options.scrollTable && !(i.style || "").includes("inline")) {
                ...
                }, t.children = [ I ], i = I.attrs;
            }
        } else if ("td" !== t.name && "th" !== t.name || !i.colspan && !i.rowspan) {
            if ("ruby" === t.name) {
                t.name = "span";
                for (var j = 0; j < s.length - 1; j++) "text" === s[j].type && "rt" === s[j + 1].name && (s[j] = {
                    name: "span",
                    attrs: {
                    ...
                    children: [ {
                        name: "div",
                        attrs: {
                            style: "font-size:50%;text-align:start;" + (s[j + 1].attrs.style || "")
                        },
                        children: s[j + 1].children
                    }, s[j] ]
                }, s.splice(j + 1, 1));
            }
        } else for (var L = this.stack.length; L--; ) if ("table" === this.stack[L].name) {
            this.stack[L].flag = 1;
            break;
        }
    } else {
    ...
    if ((c.display || "").includes("flex") && !t.c) for (var U = s.length; U--; ) {
        var V = s[U];
        V.f && (V.attrs.style = (V.attrs.style || "") + V.f, V.f = void 0);
    }
    var B = e && (e.attrs.style || "").includes("flex") && !t.c && !(c.display || "").includes("inline");
    for (var P in B && (t.f = ";max-width:100%"), c) if (c[P]) {
        var Z = ";".concat(P, ":").concat(c[P].replace(" !important", ""));
        B && (P.includes("flex") && "flex-direction" !== P || "align-self" === P || "-" === c[P][0] || "width" === P && Z.includes("%")) ? (t.f += Z, 
        "width" === P && (i.style += ";width:100%")) : i.style += Z;
    }
